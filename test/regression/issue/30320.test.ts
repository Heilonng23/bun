...
// A leading `/` in a sideEffects entry is package-root-relative, not
// filesystem-absolute. `r_fs.abs` would treat it as a new root and discard
// the package dir, silently tree-shaking the file. This reproduces on POSIX
// too (fails without the leading-separator strip, passes with it), so it is
// a genuine fail-before guard on the Linux gate.
test("#30320 bundler keeps sideEffects entries with a leading slash", async () => {
