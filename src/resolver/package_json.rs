...
impl PackageJSON {
    ...
    /// not filesystem-absolute. Strip it so `r_fs.abs` joins against the
    /// package dir instead of discarding it (`path.resolve` treats an absolute
    /// later component as a new root). Matches esbuild's `filepath.Join`. #30320
    fn strip_pattern_root(name: &[u8]) -> &[u8] {
        name.strip_prefix(b"/")
